...
The Docker image is based on the `ghcr.io/mostlygeek/llama-swap:cuda` base image, which includes llama-server and vLLM.

```bash
# Build the image
docker build -t llama-swap .

# Run the container
docker run -p 8080:8080 -v /path/to/config.yaml:/app/config.yaml llama-swap

# For GPU support:
docker run --gpus all -p 8080:8080 -v /path/to/config.yaml:/app/config.yaml llama-swap
```
